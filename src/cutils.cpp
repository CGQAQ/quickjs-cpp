...
// TODO(CGQAQ): should remove this eventually
...
int has_suffix(const char *str, const char *suffix) {
    ...
    return static_cast<int>(len >= slen &&
                            (memcmp(str + len - slen, suffix, slen) == 0));
}
...
int dbuf_realloc(DynBuf *s, size_t new_size) {
    ...
    if (new_size > s->allocated_size) {
        ...
        new_buf = static_cast<uint8_t *>(
            s->realloc_func(s->opaque, s->buf, new_size));
        if (new_buf == nullptr) {
            s->error = true;
            return -1;
        }
...
int dbuf_put(DynBuf *s, const uint8_t *data, size_t len) {
    if (((s->size + len) > s->allocated_size)) [[unlikely]] {
...
int dbuf_putstr(DynBuf *s, const char *str) {
    return dbuf_put(s, reinterpret_cast<const uint8_t *>(str), strlen(str));
}

// TODO(CGQAQ): compatibility improvement
...
dbuf_printf(DynBuf *s, const char *fmt, ...) {
    ...
    if (len < sizeof(buf)) {
        /* fast case */
        return dbuf_put(s, reinterpret_cast<uint8_t *>(buf), len);
    }
    if (dbuf_realloc(s, s->size + len + 1) != 0) {
        return -1;
    }
    va_start(ap, fmt);
    vsnprintf(reinterpret_cast<char *>(s->buf + s->size),
              s->allocated_size - s->size, fmt, ap);
    va_end(ap);
    s->size += len;
...
using exchange_f = void (*)(void *, void *, size_t);
using cmp_f = int (*)(const void *, const void *, void *);
...
static void heapsortx(void *base, size_t nmemb, size_t size, cmp_f cmp,
                      void *opaque) {
    auto *basep = static_cast<uint8_t *>(base);
    size_t i;
    size_t n;
    size_t c;
    size_t r;
...
void rqsort(void *base, size_t nmemb, size_t size, cmp_f cmp, void *opaque) {
    ...
    sp->base = static_cast<uint8_t *>(base);
    sp->count = nmemb;
    sp->depth = 0;
    sp++;
